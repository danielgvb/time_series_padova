...
```
The forecast of the GGM looks like the restaurant is loosing grip, actually restaurants can do well for way longer than the estimated period by the GGM. So we take this as a downside scenario forecast.
But lets try to forecast a baseline scenario, with the second best model: the Holt-Winters.
...
Finally, we want to use the long term forecast, so we can use the monthly combination of models for that.
### Montly
```{r}
